...
!(function (a, b) {
	...
	function fb(a) {
		...
		if (
			...
			if (
				...
				var l = new Date(c[2]),
					m = ["Ming", "Sen", "Sel", "Rab", "Kam", "Jum", "Sab"][l.getDay()];
				if (c[1].substr(0, 3) !== m)
	...
	var oe = /D[oD]?(\[[^\[\]]*\]|\s)+MMMM?/,
		...
		qe = "Jan_Feb_Mar_Apr_Mei_Jun_Jul_Agu_Sep_Okt_Nov_Des".split("_"),
		re = be,
		se = be;
	U("Y", 0, 0, function () {
		var a = this.year();
	...
	var ve = "Minggu_Senin_Selasa_Rabu_Kamis_Jumat_Sabtu".split("_"),
		we = "Ming_Sen_Sel_Rab_Kam_Jum_Sab".split("_"),
		xe = "Mi_Sen_Sel_Ra_Ka_Jum_Sa".split("_"),
		ye = be,
		ze = be,
		Ae = be;
	U("H", ["HH", 2], 0, "hour"),
	...
	var Be,
		...
		Me = /^\/?Date\((\-?\d+)/i,
		Ne =
			/^((?:Sen|Sel|Rab|Kam|Jum|Sab|Ming),?\s)?(\d?\d\s(?:Jan|Feb|Mar|Apr|Mei|Jun|Jul|Agu|Sep|Okt|Nov|Des)\s(?:\d\d)?\d\d\s)(\d\d:\d\d)(\:\d\d)?(\s(?:UT|GMT|[ECMP][SD]T|[A-IK-Za-ik-z]|[+-]\d{4}))$/;
